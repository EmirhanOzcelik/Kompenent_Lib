...
int16_t Potansiyometre::oku()
{
    ...
    if (abs(_son_saf_deger - deger) > _okuma_tolerans) // değişiklik var mı?
    {
        if (deger > _son_saf_deger)
        {
            ...
            hiz_kontrol(_degisim_miktar, max_deger_yakala(_degisim_miktar));
...
int16_t Potansiyometre::oku_map(int16_t minn, int16_t maxx)
{
    ...
    {
        _adc_maxdeger = (_oto_maxdeger_kalibre) ? max_deger_yakala(deger,4900) : _cozunurluk_deger;
        _min = minn;
        _max = maxx;
...
void Potansiyometre::test(bool izinn)
{
    ...
    uint16_t DT1 = max_deger_yakala(_degisim_miktar);
